...
podTemplate(cloud: "kubernetes", containers: [
    ...
    node(POD_LABEL) {
        //Vault Configuration
        def configuration = [vaultUrl: 'http://10.97.24.170:8200',
        ...
        withVault([configuration: configuration, vaultSecrets: secrets]) {
            ...
            stage ('Checkout') {
                try {
                    git credentialsId: 'github-key', branch: "$BRANCH_NAME", url: "git@github.com:bic-harness/demo-java.git"
                }
                catch (exc) {
            ...
            stage('Build Exporter for Monitoring') {
                container('maven') {
                    try {
                        sh """
                            wget http://bic-harness-demo.s3.eu-west-2.amazonaws.com/jmx_prometheus_javaagent-0.12.0.jar
            ...
            stage('Kaniko - Build & Ship') {
                container('kaniko') {
                    try {
                        ...
                        /kaniko/executor --dockerfile=$WORKSPACE/Dockerfile --context=$WORKSPACE  --destination=bicharness/demojava:$BUILD_NUMBER --skip-tls-verify
                        """                  
                    }
                    catch (exc) {
